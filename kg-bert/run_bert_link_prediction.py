...
from model_distillation import BertForReconstruction
...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, text_c=None, label=None, kc=None):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, 
                input_ids_reconstruction, input_mask_reconstruction, label_id_reconstruction):
        self.input_ids = input_ids
        self.input_mask = input_mask
        self.segment_ids = segment_ids
        self.label_id = label_id
        self.input_ids_reconstruction = input_ids_reconstruction
        self.input_mask_reconstruction = input_mask_reconstruction
        self.label_id_reconstruction = label_id_reconstruction
...
class KGProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, reconstruction):
        """See base class."""
        self.triplet2kc = None
        if reconstruction:
            self.triplet2kc = {}
            with open(os.path.join(data_dir, "knowledge_context.txt")) as f:
                for line in f.readlines():
                    head, tail, relation, kc = line.split(' |**| ')
                    head, tail, relation, kc = head.strip(), tail.strip(), relation.strip(), kc.strip()
    ...
    def _create_examples(self, lines, set_type, data_dir):
        ...
        for (i, line) in tqdm(enumerate(lines)):
            ...
            if set_type == "dev" or set_type == "test":
                ...
                if self.triplet2kc is not None:
                    key = (line[0], line[2], line[1])
                    if key in self.triplet2kc:
                        knowledge_context = self.triplet2kc[key]
                    else:
                        knowledge_context = random.choice(list(self.triplet2kc.values()))
                guid = "%s-%s" % (set_type, i)
                text_a = head_ent_text
                text_b = relation_text
                text_c = tail_ent_text 
                examples.append(
                    InputExample(guid=guid, text_a=text_a, text_b=text_b, text_c = text_c, label="1", kc=knowledge_context))
                ...
                if rnd <= 0.5:
                    ...
                    for j in range(5):
                        ...
                        examples.append(
                            InputExample(guid=guid, text_a=tmp_head_text, text_b=text_b, text_c = text_c, label="0", kc=knowledge_context))       
                else:
                    # corrupting tail
                    tmp_tail = ''
                    for j in range(5):
                        ...
                        examples.append(
                            InputExample(guid=guid, text_a=text_a, text_b=text_b, text_c = tmp_tail_text, label="0", kc=knowledge_context))                                                  
        return examples
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, print_info = True):
    ...
    for (ex_index, example) in tqdm(enumerate(examples)):
        ...
        kc_ids, attention_mask_reconstruction, reconsturction_label = None, None, None
        if example.kc:
            vocab_list = list(tokenizer.vocab.keys())
            kc = example.kc
            kc_tokens = tokenizer.tokenize(kc)
            reconsturction_label = random_word(kc_tokens, tokenizer, vocab_list)
            kc_tokens = ['[CLS]'] + kc_tokens + ['[SEP]']
            reconsturction_label = [-1] + reconsturction_label + [-1]
            kc_ids = tokenizer.convert_tokens_to_ids(kc_tokens)
            assert len(kc_ids) == len(reconsturction_label)
            attention_mask_reconstruction = [1 for _ in kc_ids]
            while len(kc_ids) < max_seq_length:
                kc_ids.append(0)
                attention_mask_reconstruction.append(0)
                reconsturction_label.append(-1)
            kc_ids = kc_ids[:max_seq_length]
            attention_mask_reconstruction = attention_mask_reconstruction[:max_seq_length]
            reconsturction_label = reconsturction_label[:max_seq_length]
        ...
        features.append(
                InputFeatures(input_ids=input_ids,
                            ...
                            label_id=label_id,
                            input_ids_reconstruction=kc_ids,
                            input_mask_reconstruction=attention_mask_reconstruction,
                            label_id_reconstruction=reconsturction_label
                )                                
        )
    return features

def random_word(tokens, tokenizer, vocab_list):
    ...
    for i, token in enumerate(tokens):
        ...
        if prob < 0.15:
            ...
            try:
                ...
                output_label.append(tokenizer.convert_tokens_to_ids(["[UNK]"])[0])
...
def main():
    ...
    parser.add_argument('--reconstruction', type=bool, default=False, help="Whether to add reconstruction as an auxiliary task")
    ...
    if args.do_train:
        train_examples = processor.get_train_examples(args.data_dir, args.reconstruction)
    ...
    model = BertForReconstruction.from_pretrained(args.bert_model,
              cache_dir=cache_dir,
    ...
    if args.do_train:
        ...
        # all_input_ids_reconstruction, all_input_mask_reconstruction, all_label_ids_reconstruction = None, None, None
        if args.reconstruction:
            all_input_ids_reconstruction = torch.tensor([f.input_ids_reconstruction for f in train_features], dtype=torch.long)
            all_input_mask_reconstruction = torch.tensor([f.input_mask_reconstruction for f in train_features], dtype=torch.long)
            all_label_ids_reconstruction = torch.tensor([f.label_id_reconstruction for f in train_features], dtype=torch.long)

            train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids,
                                    all_input_ids_reconstruction, all_input_mask_reconstruction, all_label_ids_reconstruction)
        else:
            train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids)
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids_rec, input_mask_rec, label_ids_rec = None, None, None
                if args.reconstruction:
                    input_ids, input_mask, segment_ids, label_ids, input_ids_rec, input_mask_rec, label_ids_rec = batch
                else:
                    input_ids, input_mask, segment_ids, label_ids = batch
                # define a new function to compute loss values for both output_modes
                loss = model(input_ids, segment_ids, input_mask, labels=label_ids,
                                input_ids_reconstruction=input_ids_rec, attention_mask_reconstruction=input_mask_rec, labels_reconstruction=label_ids_rec
                )
            ...
            os.makedirs(os.path.join(args.output_dir, 'Epoch{}'.format(str(epoch))))
            model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
            output_model_file = os.path.join(args.output_dir,'Epoch{}'.format(str(epoch)), WEIGHTS_NAME)
            output_config_file = os.path.join(args.output_dir,'Epoch{}'.format(str(epoch)), CONFIG_NAME)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = BertForReconstruction.from_pretrained(args.output_dir, num_labels=num_labels)
        tokenizer = BertTokenizer.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
    else:
        model = BertForReconstruction.from_pretrained(args.bert_model, num_labels=num_labels)
    model.to(device)
