...
class KGCPromptTuner(pl.LightningModule):
    def __init__(self, configs, text_dict, gt):
        ...
        self.history = {'perf': ..., 'loss': [], 'kgc loss':[], 'performance': []}
    ...
    def forward(self, ent_rel, src_ids, src_mask, ent_rel_kc=None, kc_ids=None, kc_mask=None):
        bs = ent_rel.size(0)
        ...
        prompt_kc=None
        if ent_rel_kc is not None:
            ent_kc, tail_kc, rel_kc = ent_rel_kc[:, 0], ent_rel_kc[:, 1], ent_rel_kc[:, 2]
            ent_embed_kc = all_ent_embed[ent_kc]
            rel_embed_kc = all_rel_embed[rel_kc]
            tail_embed_kc = all_ent_embed[tail_kc]
            prompt_kc = self.prompter(torch.stack([ent_embed_kc, rel_embed_kc, tail_embed_kc], dim=1))
            prompt_attention_mask_kc = torch.ones(ent_embed_kc.size(0), self.configs.prompt_length * 3).type_as(kc_mask)
            kc_mask = torch.cat((prompt_attention_mask_kc, kc_mask), dim=1)

        output = self.plm(input_ids=src_ids, attention_mask=src_mask, layerwise_prompt=prompt,
                        input_ids_kc=kc_ids, kc_mask=kc_mask, layerwise_prompt_kc=prompt_kc,
        )
        ...
        if ent_rel_kc is not None:
            return logits, pred, prediction_scores
    ...
    def training_step(self, batched_data, batch_idx):
        ...
        if self.configs.reconstruction:
            kc_ids=batched_data['kc_ids']
            kc_mask=batched_data['kc_mask']
            kc_labels=batched_data['kc_labels']
            ent_rel_kc=batched_data['ent_rel_kc']
        
        if self.configs.reconstruction:
            logits, pred, prediction_scores = self(ent_rel, src_ids, src_mask, ent_rel_kc, kc_ids, kc_mask)
        else:
            logits, pred = self(ent_rel, src_ids, src_mask)
        
        loss = self.loss_fn(logits, labels)

        self.history['kgc loss'].append(loss.detach().item())
        
        if self.configs.reconstruction:
            prompt_label_kc = -1*torch.ones(kc_labels.size(0), self.configs.prompt_length * 3).type_as(kc_labels)
            kc_labels = torch.cat((prompt_label_kc, kc_labels), dim=1)
            # cal reconstruction loss
            # print(prediction_scores.shape)
            # print(kc_labels.shape)
            # print(kc_labels)
            loss_fct_mlm = nn.CrossEntropyLoss(ignore_index=-1)  # -100 index = padding token
            masked_lm_loss = loss_fct_mlm(prediction_scores.view(-1, self.plm.config.vocab_size), kc_labels.view(-1))
            loss += masked_lm_loss
    ...
    def validation_epoch_end(self, outs):
        ...
        print(perf)
        self.history['performance'] = list(perf.loc['mean ranking'])
