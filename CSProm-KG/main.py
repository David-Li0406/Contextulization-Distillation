...
def main():
    ...
    # if configs.wandb:
    #     logger = pl.loggers.WandbLogger(project="CSProm-KG", name=configs.dataset)
        # logger.log_hyperparams(vars(configs))
    ...
    triplet2kc = {}
    with open(os.path.join(configs.dataset_path, configs.dataset, 'knowledge_context{}.txt'.format(configs.suffix))) as f:
        for line in f.readlines()[1:]:
            if 'ICEWS' in configs.dataset:
                h, t, r, time, kc = line.split(" |**| ")
                h, t, r, time, kc = h.strip(), t.strip(), r.strip(), time.strip(), kc.strip()
                triplet2kc[(h,t,r,time)] = kc
            else:
                h, t, r, kc = line.split(" |**| ")
                h, t, r, kc = h.strip(), t.strip(), r.strip(), kc.strip()
                triplet2kc[(h,t,r)] = kc

    ## construct datamodule
    datamodule = KGCDataModule(configs, train, valid, test, text_dict, tok, gt, triplet2kc)
    ...
    if configs.model_path == '':
        ...
        import json
        with open('{}_{}.json'.format(configs.dataset, str(configs.reconstruction)), 'w') as f:
            f.write(json.dumps(model.history['performance']))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-reconstruction', default=False, type=bool, help='')
    parser.add_argument('-suffix', default='', type=str, help='')
    parser.add_argument('-wandb', default=True, type=bool, help='')
